...
bool Camera::cube_collision(glm::vec3 next_position, float t, Bounds bounds) const
{
    ...
    if (next_position.x >= bounds.x[1])
    {
        return true;
    }

    if (next_position.y <= bounds.y[0])
    {
        return true;
    }

    if (next_position.y >= bounds.y[1] - 0.1f)
    {
        return true;
    }

    if (next_position.z <= bounds.z[0])
    {
        return true;
    }

    if (next_position.z >= bounds.z[1])
    {
        return true;
    }
    return false;
